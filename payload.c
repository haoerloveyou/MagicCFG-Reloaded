...
int get_payload_configuration(uint16_t cpid, const char* identifier, checkm8_32_t* config) {
    ...
    switch(cpid) {
        case 0x8950:
            ...
            if(!trampoline) {
                printf("ERROR: Failed to build payload trampoline.\n");
                return -1;
            }
    ...
    if((cpid|0xf) == 0x895f){
        ...
        if(ret != 0) {
            printf("ERROR: Failed to add offsets to payload.\n");
            return -1;
        }
        ...
        if(ret != 0) {
            printf("ERROR: Failed to add trampoline to payload.\n");
            return -1;
        }
    }
...
int gen_limera1n(int cpid, int rom, unsigned char** payload, size_t* payload_len) {
    ...
    *payload_len = limera1n_payload_len;
    ...
    switch(cpid) {
        ...
        case 0x8930:
            ...
            *(uint32_t*)exploit_lr = 0x8403BF9C;
            break;
        default:
            printf("ERROR: No payload offsets are available for this device.\n");
            return -1;
    }
    ...
    if(ret != 0) {
        printf("ERROR: Failed to add exploit_lr to payload.\n");
        return -1;
    }
