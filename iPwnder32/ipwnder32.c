...
static int irecv_get_device() {
    if(clientpwn) {
        irecv_close(clientpwn);
        clientpwn = NULL;
    }
    irecv_open_with_ecid(&clientpwn, 0);
    if(!clientpwn) {
        return -1;
    }
    int mode = 0;
    
    irecv_get_mode(clientpwn, &mode);
...
static void print_devinfo(irecv_device_t device, const struct irecv_device_info *devinfo){
    ...
    if (p) {
        ...
        char* pend = strchr(p, ']');
        if (pend) {
            printf("PWND:[\x1b[35;1m%.*s]", (int)(pend-p), p);
        }
    }
    printf("\n");
...
static void usage(char** argv) {
    ...
    printf("\t<NOTE> If you use s5l895Xx, your device will automatically load pwned iBSS.\n");
    printf("\t\t\t[Support device lists]\n");
    printf("\t\t\t  \x1b[36ms5l8920x - limera1n\n");
    printf("\t\t\t  \x1b[36ms5l8922x - limera1n\n");
    printf("\t\t\t  \x1b[36ms5l8930x - limera1n\n");
    printf("\t\t\t  \x1b[36ms5l895Xx - \x1b[35mcheckm8\n");
    printf("\t\t\t  \x1b[36ms5l8960x - \x1b[35mcheckm8\n");
    printf("\t\t\t[Additional flag]\n");
    printf("\t\t\t  \x1b[35m--noibss: Do not enter pwnediBSS (s5l895Xx only)\n");
    printf("\n");
    ...
    printf("\t\t\t[Support device lists]\n");
    printf("\t\t\t  \x1b[36miPhone 5 [iPhone5,2] - \x1b[35m(iOS 10.3.4)\n");
    printf("\n");
#endif
}


int main_LLLL(int argc, char** argv) {
    int r;
    const char* ibsspath;
    const char* ibecpath;
    ...
    if(!strcmp(argv[1], "-p")) {
        ...
        fp = fopen(argv[2], "rb");
        if(!fp) {
            printf("\x1b[31;1mERROR: opening %s!\n", argv[2]);
            return -1;
        }
    ...
    // get device
    int i = 0;
    printf("\x1b[36mWaiting for device in DFU mode...\n");
    while(irecv_get_device() != 0) {
        ...
        i++;
        if(i>=5){
            printf("ERROR: Failed to get DFU device.\n");
            return -1;
        }
#endif
    }
    ...
    const struct irecv_device_info *devinfo = irecv_get_device_info(clientpwn);
    irecv_devices_get_device_by_client(clientpwn, &device);
    ...
    if(!devinfo->cpid){
        printf("ERROR: Failed to get CPID form DFU device.\n");
        irecv_close(clientpwn);
        return -1;
    }
    ...
    if((devinfo->cpid|0xf) == (0x8950|0xf) && no_pwnibss != 1){
        ...
        if (!d) {
            ...
            r = mkdir("image3/", 0755);
            if(r != 0){
                printf("\x1b[31;1mERROR: opening dir: %s!\n", outdir);
            }
        }
        if(init_dl() != 0){
            return -1;
    ...
    if(send == 1){
        // ibss/illb
        if(!(devinfo->srtg)){
            printf("ERROR: This device is in soft (LLB/iBSS) DFU mode.\n");
            printf("please put device in normal DFU mode.\n");
            irecv_close(clientpwn);
            return 0;
        }
        
        if((devinfo->cpid|0xf) == (0x8950|0xf)){
            boot_client(clientpwn, file, file_len);
        } else {
            boot_client_n(clientpwn, file, file_len);
        }
    ...
    if(pwndfu == 1){
        ...
        if((devinfo->cpid|0xf) == (0x8920|0xf) || devinfo->cpid == 0x8930){
            ...
            limera1n_exploit(clientpwn, device, devinfo);
        } else if((devinfo->cpid|0xf) == (0x8950|0xf)){
            //printf("checkm8_32\n");
            r = checkm8_32_exploit(clientpwn, device, devinfo);
            pwnibss = 1;
        } else if(devinfo->cpid == 0x8960){
            //printf("checkm8_32\n");
            checkm8_32_exploit(clientpwn, device, devinfo);
        } else {
    ...
    if(pwnibss == 1 && no_pwnibss != 1){
        clientpwn = NULL;
        usleep(100);
        irecv_open_with_ecid_and_attempts(&clientpwn, 0, 5);
        if(!clientpwn) {
            printf("ERROR: Failed to reconnect to device.\n");
            return -1;
        }
        
        devinfo = irecv_get_device_info(clientpwn);
        irecv_devices_get_device_by_client(clientpwn, &device);
        ...
        fd = fopen(path, "r");
        if (!fd) {
            printf("\x1b[31;1mERROR: opening image: %s!\n", path);
            irecv_close(clientpwn);
            return -1;
        }
        ...
        if(TBX != 1){
            ...
            clientpwn = NULL;
            usleep(1000);
            irecv_open_with_ecid_and_attempts(&clientpwn, 0, 10);
            if(!clientpwn) {
                printf("ERROR: Failed to reconnect to device.\n");
                return -1;
            }
            printf("\x1b[31;1mDevice is now in pwned iBSS mode!\n");
            irecv_close(clientpwn);
#ifdef HAVE_HOOKER
        }
#endif
    ...
    if(TBX == 1 && (devinfo->cpid|0xf) == (0x8950|0xf)){
        ...
        if((devinfo->srtg)){
            printf("ERROR: This device is not in soft (iBSS) DFU mode.\n");
            irecv_close(client);
            return 0;
        }
...
#include "hooker.h"
                ...
                printf("\x1b[31;1mBooting...\n");
                
                r = irecv_send_command(client, "go");
                if(r != 0) {
                    printf("ERROR: Failed to execute payload.\n");
                    return -1;
                }
...
int pwnA6A7() {
    ...
    if(pwndfu == 1){
        ...
        if((devinfo->cpid|0xf) == (0x8920|0xf) || devinfo->cpid == 0x8930){
            ...
            //printf("checkm8_32\n");
            return checkm8_32_exploit(clientpwn, device, devinfo);
        } else if(devinfo->cpid == 0x8960){
            //printf("checkm8_32\n");
            return checkm8_32_exploit(clientpwn, device, devinfo);
        } else {
