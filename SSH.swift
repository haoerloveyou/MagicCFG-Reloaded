...
public class SSH {
    ...
    public func capture_data(_ command: String) throws -> (status: Int32, output: Data) {
        var ongoing = Data()
        let status = try execute_data(command) { (output) in
            ongoing += output
        }
        return (status, ongoing)
    }
    ...
    /// - Returns: exit code of the command
    /// - Throws: SSHError if the command couldn't be executed
    public func execute_data(_ command: String, output: ((_ output: Data) -> ())) throws -> Int32 {
