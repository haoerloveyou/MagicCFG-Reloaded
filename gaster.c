...
gaster_checkm8(usb_handle_t *handle) {
	...
	while(stage != STAGE_PWNED && wait_usb_handle(handle, 0, 0, checkm8_check_usb_device, &pwned)) {
		if(!pwned) {
			...
			stage = STAGE_PWNED;
			printf("Now you can boot untrusted images.");
		}
...
//		printf("env:");
//		printf("USB_TIMEOUT - USB timeout in ms");
//		printf("options:");
//		printf("pwn - Put the device in pwned DFU mode");
//		printf("load iBSS iBEC - Load the untrusted boot chain");
//		printf("decrypt src dst - Decrypt file using GID0 AES key");
//	}
//	return ret;
//}
